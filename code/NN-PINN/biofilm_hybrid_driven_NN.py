...
from models_hybrid_driven_NN import FCResNet, PDE_residual, PorosityPredictor, NN_c, read_matrix_data, HuberLoss
from save_method_hybrid_NN import save_training_artifacts
...
co1 = torch.tensor(1.0).to(device)
#co1 = torch.tensor(0.108).to(device)   # The optimal co1, co1.requires_grad should be set to False
co1.requires_grad = True
...
config = {
    ...
    "Main code name": "biofilm_hybrid_driven_NN.py",
    "Model code name": "models_hybrid_driven_NN.py",
    "The code used for saving is": "save_method_hybrid_NN.py",
...
# Load the pre-trained model as the parameter initialization setting
...
optimizer2 = torch.optim.Adam(params2, lr=1e-4, weight_decay= weight_decay)
...
for epoch in range(epoch_number):
    ...
    for (batch_idx1, data_label), (batch_idx2, data_pde) in tqdm(
        ...
        if epoch <= epoch_change:
            ...
            pde_residual = PDE_residual(data_pde, model,model_D,
                                        alpha,co1,
                                        xmax, xmin,
                                        ymax, ymin,
                                        tmax, tmin,
            ...
            #loss = loss_data_driven * scale_p + loss_initial_M + loss_pde  # NN-PINN: only M0
...
''')
        ...
        if R2_timeout > R2_best:
            ...
            save_training_artifacts(model,model_D,loss_Total, config, model_name="hybrid_driven_parameters_NN", best = True)
...
loss_Total = np.array(loss_total)
save_training_artifacts(model,model_D,loss_Total, config, model_name = 'hybrid_driven_parameters_NN', best = False)
