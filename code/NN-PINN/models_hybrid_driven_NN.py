...
class PDE_residual(nn.Module):
    ...
    Args:
        ...
        model : Main model
        model_D : Alternative model for the self-inhibition factor
    ...
    """
    def __init__(self, data_pde, model, model_D, alpha, co1,
        ...
        self._validate_inputs(data_pde, model, model_D)
    ...
    def _validate_inputs(self, data_pde, model, model_D):
        ...
        if not isinstance(model, nn.Module):
            raise TypeError("The main model must be of the nn.Module type !")
        
        if not isinstance(model_D, nn.Module):
            raise TypeError("The model_D must be of the nn.Module type !")
    ...
    def _compute_growth_kinetics(self):
        ...
        # Calculate the normalized biomass (M/M_max)
        M_real_norm = self.M_real/10.0

        # # self-inhibition factor（Alternative model）
        self.pw = torch.zeros_like(M_real_norm).float()
        mask0 = (self.M_real > 0.01).squeeze()
        self.pw[mask0] = self.NN_D(M_real_norm[mask0])
