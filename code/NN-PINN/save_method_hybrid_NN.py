# 生物膜生长方程中的扩散系数、生物膜反应项（包括生长项和死亡项）用杨老师组提出的模型，n-M本构关系用NN替代
import torch
import json
import os
import pandas as pd
...
def save_training_artifacts(model,
                            ...
                            loss_Total,
                            config,
                            model_name="hybrid_driven_parameters_NN",
                            best = False):
    """
    ...
    save_dir = f"/data1/yq1/biofilm_zgf/result/without_s_curve/{model_name}_{timestamp}"
    ...
    if best == False:    
        ...
        if model_name == "data_driven_parameters":
            torch.save({'model_dict':model.state_dict()},save_parameters +'parameters.pt')
        elif model_name == "hybrid_driven_parameters_NN":
            torch.save({'model_dict':model.state_dict()},save_parameters +'parameters.pt')
            torch.save({'model_dict':model_D.state_dict()},save_parameters+'D.pt')
